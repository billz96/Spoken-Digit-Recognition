...
if not os.path.exists(model_path):
    ...
    model = Sequential()
    model.add(
        LSTM(50, return_sequences=True, input_shape=INPUT_SHAPE, dropout=DROPOUT)
    ) # 256 old val, 50 new val
    model.add(Flatten())
    model.add(Dense(100, activation='relu'))  # 128 old, 100 test val
...
# testing data from data/16bit folder
test_files = glob.glob("./data/spoken_numbers_pcm/" + "*.wav")
test_files = sorted(test_files)

X_tests, y_tests = take_nXs(test_files, 120) # take 120 random and formated data
predictions = model.predict_classes(X_tests, batch_size=10, verbose=0) # predict output

# show the "shape" of predictions and labels
print("\npredictions size: {}".format(predictions.shape))
print("y-tests size: {}".format(len(y_tests)))
...
for i in range(120):
    if predictions[i] == y_tests[i]:
        correct_guesses += 1
accuracy = (correct_guesses / 120) * 100
print("accuracy: {}%\n".format(accuracy))



# tesing data from data/tests folder
file_path = "./data/tests/output.wav"
